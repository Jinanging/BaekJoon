...
//    단어장에 단어가 반드시 1개 이상 존재하는 입력만 주어진다.
    ...
    public static void main(String[] args) throws IOException {
        ...
        HashMap<String, Integer> wordSheet = new HashMap<>();
        ...
        for(int i = 0; i < number; i++){
            ...
            if(word.length() >= stand){
                if(!wordSheet.containsKey(word)){
                    wordSheet.put(word,1);
                }
                else{
                    int count = wordSheet.get(word);
                    count++;
                    wordSheet.put(word, count);
                }
        ...
        // sorting
        List<String> words = new ArrayList<>(wordSheet.keySet());

        words.sort((w1, w2) -> {
            int freqCompare = wordSheet.get(w2) - wordSheet.get(w1); // 빈도 내림차순
            if (freqCompare != 0) return freqCompare;

            int lenCompare = w2.length() - w1.length(); // 길이 내림차순
