...
//        1 초	128 MB	6396	3456	2812	54.783%
//                문제
...
//        로그 엔트리의 개수 m을 나타내는 4 개의 정수가 주어진다. 여기서, 3 ≤ n, k ≤ 100,
//        1 ≤ t ≤ n, 3 ≤ m ≤ 10,000이다. 그 다음 m개의 줄에는 각 풀이에 대한 정보가 제출되는 순서대로 주어진다.
...
//        출력은 표준출력을 사용한다. 주어진 각 테스트 데이터에 대해 당신 팀의 순위를 한 줄에 출력하여야 한다
        ...
        for(int i = 0; i < testCaseCount; i++){
            ...
            for(int j = 0; j < teamCount; j++){
                teams[j] = new Team();
                teams[j].scores = new int[problemCount + 1];
                teams[j].id = j + 1;
            }
            ...
            for(int j = 0; j < logCount; j++){
                ...
                int problemId = Integer.parseInt(st2.nextToken());
                ...
                t.scores[problemId] = Math.max(t.scores[problemId], score);
            }

            for(int j = 0; j < teamCount; j++){
                int sum = 0;
                for(int p = 1; p <= problemCount; p++){
                    sum += teams[j].scores[p];
                }
                teams[j].total = sum;
            }
            ...
            Arrays.sort(teams, (team1, team2) -> {
                if(team1.total != team2.total){
                    return team2.total - team1.total;
                }
                if(team1.submitCount != team2.submitCount) {
                    return team1.submitCount - team2.submitCount;
                }


                return team1.lastTime - team2.lastTime;
            });
