...
//        번호를 부여할 땐 키가 가장 작은 아이가 1번, 그 다음이 2번, ... , 가장 큰 아이가 20번이 된다. 강산이네 반 아이들은 항상 20명이며, 다행히도 같은 키를 가진 학생은 한 명도 없어서 시간이 조금 지나면 아이들은 자기들의 번호를 인지하고 한 줄로 세우면 제대로 된 위치에 잘 서게 된다.
//
...
//        각각의 테스트 케이스에 대해 테스트 케이스의 번호와 학생들이 뒤로 물러난 걸음 수의 총합을 공백으로 구분하여 출력한다.
        ...
        int[][] state = new int[testCase][21];


        // 초기 정보를 담은 2차원 배열 생성 및 입력 (완)

        for(int i = 0; i < testCase; i++){
            state[i][0] = scanner.nextInt();
            for(int j= 0; j < 20; j++){
                state[i][j+1] = scanner.nextInt();
            }
        }
        ...
        for (int i = 0; i < testCase; i++) {
            ...
            for (int j = 0; j < 20; j++) {
                heights[j] = state[i][j + 1];
            }

            int result = Counting(heights);
            System.out.println(state[i][0] + " " + result);

        }

        // 정렬 -> sorting? 어차피 각 테스트 케이스부터 해야되는거 함수로 만들자
    }

    public static int Counting(int[] height){
        int size = 0;
        int count = 0;
        int[] sortingArray = new int[20];

        // 학생들이 들어오는 순서대로 처리
        for(int i = 0; i < height.length; i++){

            // 기본적으로는 맨 뒤에 서는 것이 가능
            int possible = size;

            // 자기 앞에 자기보다 키가 큰 학생이 있는지 확인
            // 있다면 그 중 가장 앞에 있는 학생 위치를 찾는다
            for(int j = 0; j < size; j++){
                if(sortingArray[j] > height[i]){
                    possible = j;
                    break;
                }
            }

            // A부터 그 뒤의 모든 학생들은 공간을 만들기 위해 한 발씩 뒤로 물러선다
            for(int j = size; j > possible; j--){
                sortingArray[j] = sortingArray[j - 1];
                count++; // 한 명이 뒤로 물러설 때마다 카운트 증가
            }

            // 해당 위치에 현재 학생을 세운다
            sortingArray[possible] = height[i];
            size++; // 줄에 선 학생 수 증가
        }
