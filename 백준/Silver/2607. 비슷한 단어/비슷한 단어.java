...
//        2
        ...
        String[] word = new String[loop];

        for (int i = 0; i < loop - 1; i++) {
            word[i] = br.readLine();
           if( similarTest(origin, word[i])){
               count++;
           }
        }
    ...
    public static boolean similarTest(String origin, String compare) {
        ...
        for(int i = 0; i < 26; i++){
            diff += Math.abs(originCount[i] - compareCount[i]);
        }
        ...
        if(diff == 2 && origin.length() == compare.length()) {
            return true;
        }
